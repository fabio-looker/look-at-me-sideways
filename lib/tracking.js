...
exports = module.exports = async function({
	...
	const crypto = require('crypto');
	let valid=false, enabled=false, save=false, userHash, licenseKey, gaGuid,
		privacyPolicy = fs.readFileSync(path.resolve(__dirname, 'PRIVACY.md'),'utf-8'),
		privacyDigest = 'TODO';
	{ /* Process prefs */
		let userEmailOrHash, guids;
		const savedPrefs = (()=>{
		...
		if(cliArgs.reporting){
			...
			if(!userEmailOrHash){
				...
				throw "Invalid email or hash for 'report-user' argument.";
			}
		} else {
			{valid,enabled,userHash} = savedPrefs
		}
		guid = guids[userHash]||newGuid()
