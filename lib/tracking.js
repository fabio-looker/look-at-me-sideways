...
exports = module.exports = async function({
	...
	return {
		...
		track: ({messages, errors})=>{
			...
			let encode = (o) => Object.keys(o).map((key) => key + '=' + encodeURIComponent(staticParameters[key])).join('&');
			hits.push(encode({
				...staticParameters,
				cd1: userHash,
				cd2: licenseKey,
			...
			let rollup = messages.reduce(groupBy({grouping: ['rule', 'level', 'isExempt']}), null);
			for (let rule of rollup.subgroups) {
				for (let level of rule.subgroups) {
					for (let isExempt of level.subgroups) {
						hits.push(encode({
							...staticParameters,
							ec: 'Rule Result', 					// event category
							ea: level.key,						// event action
							ev: isExempt.summaries.count,		// event value
							cd3: rule.key,
							cd4: isExempt.key,
			...
			// insert logic for 20 payload chunks
			request();
		},
	};
